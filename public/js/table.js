...
// eslint-disable-next-line max-statements
  const gameSender = function() {
    ...
    if (team1P1Id === 'null' || team2P1Id === 'null') {
      return Materialize.toast('Please enter a player 1 for both teams', 3500);
    }

    if (team1P2Id === 'null' && team2P2Id !== 'null' || team1P2Id !== 'null' && team2P2Id === 'null') {
      return Materialize.toast('Please enter a player 2 for both teams', 3500);
    }

    if (!team1Score || !team2Score) {
      return Materialize.toast('Please enter a score for both teams', 3500);
    }

    if (team1Score === team2Score) {
      return Materialize.toast('No ties allowed!', 3500);
    }
    ...
    $xhrGame.done(() => {
      Materialize.toast('Game was submitted successfully!', 3500);
      $('#leaderboard').empty();
      $('#recentGames').empty();
      tableBuilder();
      cardBuilder();
